...
df_vendas = pd.merge(df_vendas.reset_index(),
                     df_produtos[['produto', 'preco']],
                     on='produto').set_index('data')
df_vendas['comissão'] = df_vendas['preco']*PERCENTUAL_COMISSAO
...
analise_selecionada = st.sidebar.selectbox('Analisar:',
                     COLUNAS_ANALISE)
fig = px.pie(df_vendas, values='preco', names=analise_selecionada)
